...
# ---------------------------------------------------------------- player styles --
# Player-style metadata is keyed by player_id and lives in two places:
#   data/metadata/players/<id>.json      (canonical, hand-curated)
#   app/data/players.json                (bundle generated by build_metadata)
# We prefer the canonical files. If the directory doesn't exist (no
# submissions approved yet), we fall back to the bundle so the report
# still works whenever metadata starts to land.

PLAYER_META_DIR = ROOT / "data" / "metadata" / "players"
PLAYERS_BUNDLE = ROOT / "app" / "data" / "players.json"


def load_player_metadata():
    """Return {player_id: metadata_dict} for everyone we have a style on.
    metadata_dict has the keys batting_hand, bowling_type, pace_type,
    spin_type, bowling_arm, angle_to_rhb, notes (any subset)."""
    out = {}
    if PLAYER_META_DIR.exists():
        for f in PLAYER_META_DIR.glob("*.json"):
            try:
                d = json.loads(f.read_text())
                pid = int(d.get("player_id") or f.stem)
                m = d.get("metadata") or {}
                if any(v not in (None, "") for v in m.values()):
                    out[pid] = m
            except Exception:
                continue
    if not out and PLAYERS_BUNDLE.exists():
        try:
            d = json.loads(PLAYERS_BUNDLE.read_text())
...
def batting_tag(meta):
    """RHB / LHB / None — only `unknown`/missing returns None."""
    h = (meta or {}).get("batting_hand")
    if h == "right":
        return "RHB"
    if h == "left":
        return "LHB"
...
# Pace-speed mapping. Schema enum is fast/medium/slow; user-requested
# code set is F / MF / M (fast / medium-fast / medium).
PACE_CODE = {"fast": "F", "medium": "MF", "slow": "M"}


def bowling_tag(meta):
    """Compact bowling-action code, or None if we don't have enough info.

    Pacers:                <arm><speed>      e.g. RF / RMF / RM / LF / LMF / LM
    Right-arm spin wrist:  LS                (leg-spin)
    Right-arm spin finger: OS                (off-spin)
    Left-arm spin finger:  SLA               (slow left-arm orthodox)
    Left-arm spin wrist:   LAU               (left-arm unorthodox / chinaman)
    """
    if not meta:
        return None
    bt = meta.get("bowling_type")
    if bt in (None, "", "none", "unknown"):
        return None
    arm = meta.get("bowling_arm")
    if arm not in ("right", "left"):
        return None
    arm_letter = "R" if arm == "right" else "L"
    if bt == "pace":
        speed = PACE_CODE.get(meta.get("pace_type"))
        if not speed:
            return None
        return f"{arm_letter}{speed}"
    if bt == "spin":
        st = meta.get("spin_type")
        if arm == "right":
            return {"wrist": "LS", "finger": "OS"}.get(st)
        if arm == "left":
            return {"finger": "SLA", "wrist": "LAU"}.get(st)
    return None


def bowling_category(tag):
    """'pace' / 'spin' / None given a code from bowling_tag()."""
    if not tag:
        return None
    if tag in ("OS", "LS", "SLA", "LAU"):
        return "spin"
    return "pace"
...
# ---------------------------------------- team-level style profile ---------

def team_style_profile(conn, club_id, team_ids, last_n_seasons,
                        today_iso, player_meta,
                        comp_types=("League","Cup")):
    """Walk every long-form 1st-XI L+C match in the analysis window and
    compute the team's style profile, with explicit coverage % everywhere.

    Returns:
      {
        "match_n":   total matches in scope,
        "bowl_mix": {           # by deliveries (legal balls equivalent)
            "pace_balls": int, "spin_balls": int, "unknown_balls": int,
            "pct_pace": float, "pct_spin": float, "pct_unknown": float,
            "coverage_pct": float,    # share of balls bowled by tagged
                                       # bowlers (i.e. 100 - %unknown)
        },
        "bowl_granular": [       # one entry per code, sorted by balls desc
            {"code": "RM", "balls": .., "pct": .., "category": "pace"}, ...
        ],
        "lineup": {
            "lhb_per_xi_avg": float,   # mean LHB count per played match
            "n_matches_with_data": int,
            "coverage_pct": float,     # mean fraction of XI with bat-hand
                                        # metadata, across matches
        },
        "openers_bat": {
            "lhb_present_pct": float,  # % matches with ≥1 LHB opener
            "n_matches_with_data": int,
            "coverage_pct": float,     # fraction of openers with metadata
        },
        "openers_bowl": {
            "left_arm_present_pct": float,
            "spin_present_pct": float,
            "n_matches_with_data": int,
            "coverage_pct": float,
        },
      }
    ...
    if not match_ids:
        return None
    mid_ph = ",".join("?" * len(match_ids))

    # ---- Bowling mix: aggregate balls bowled by tagged category --------
    bowl_rows = conn.execute(f"""
        SELECT bowler_id, overs, COALESCE(wides,0)+COALESCE(no_balls,0) AS extras
        FROM bowling
        WHERE match_id IN ({mid_ph})
          AND team_bowling_club_id = ?
          AND bowler_id IS NOT NULL
    """, (*match_ids, club_id)).fetchall()

    pace_balls = spin_balls = unk_balls = 0
    granular = {}
    for bid, overs, extras in bowl_rows:
        balls = overs_to_balls(overs)
        if balls <= 0:
            continue
        meta = player_meta.get(bid)
        tag = bowling_tag(meta)
        ...
        if cat == "pace":
            ...
            unk_balls += balls
        if tag:
            g = granular.setdefault(tag, {"code": tag, "balls": 0,
                                           "category": cat})
            g["balls"] += balls

    total_balls = pace_balls + spin_balls + unk_balls
    bowl_mix = {
        "pace_balls": pace_balls,
        "spin_balls": spin_balls,
        "unknown_balls": unk_balls,
        "pct_pace":  100 * pace_balls / total_balls if total_balls else 0,
        "pct_spin":  100 * spin_balls / total_balls if total_balls else 0,
        "pct_unknown": 100 * unk_balls / total_balls if total_balls else 0,
        "coverage_pct": 100 * (pace_balls + spin_balls) / total_balls
                        if total_balls else 0,
        "total_balls": total_balls,
    }
    bowl_granular = sorted(granular.values(),
                            key=lambda x: -x["balls"])
    for g in bowl_granular:
        g["pct"] = 100 * g["balls"] / total_balls if total_balls else 0

    # ---- Lineup: avg LHB per XI ----------------------------------------
    xi_rows = conn.execute(f"""
        SELECT match_id, player_id
        FROM match_players
        WHERE match_id IN ({mid_ph}) AND club_id = ?
    """, (*match_ids, club_id)).fetchall()
    by_match = {}
    for mid, pid in xi_rows:
        by_match.setdefault(mid, set()).add(pid)
    lhb_counts = []
    coverage_fracs = []
    for mid, pids in by_match.items():
        # cap at 11 — some matches list a 12th man / sub
        pids = list(pids)[:14]
        with_meta = [p for p in pids if p in player_meta
                      and (player_meta[p].get("batting_hand") or "")
                            in ("right", "left")]
        lhbs = sum(1 for p in with_meta
                   if player_meta[p].get("batting_hand") == "left")
        # Coverage on a per-match basis = fraction of XI with bat-hand.
        denom = min(len(pids), 11) or 1
        coverage_fracs.append(len(with_meta) / denom)
        lhb_counts.append(lhbs)
    lineup = {
        "lhb_per_xi_avg": (sum(lhb_counts) / len(lhb_counts)
                           if lhb_counts else 0),
        "n_matches_with_data": len(lhb_counts),
        "coverage_pct": (100 * sum(coverage_fracs) / len(coverage_fracs)
                         if coverage_fracs else 0),
    }

    # ---- Openers: batting -----------------------------------------------
    open_bat = conn.execute(f"""
        SELECT match_id, position, batsman_id
        FROM batting
        WHERE match_id IN ({mid_ph})
          AND team_batting_club_id = ?
          AND position IN (1,2)
    """, (*match_ids, club_id)).fetchall()
    by_m = {}
    for mid, pos, pid in open_bat:
        by_m.setdefault(mid, {})[pos] = pid
    matches_with_two = 0
    matches_with_lhb = 0
    coverage_known = 0
    coverage_total = 0
    for mid, pp in by_m.items():
        ids = [pp.get(1), pp.get(2)]
        ...
        any_lhb = False
        for pid in ids:
            h = (player_meta.get(pid) or {}).get("batting_hand")
            if h in ("right", "left"):
                n_known += 1
                if h == "left":
                    any_lhb = True
        coverage_known += n_known
        if n_known == 2:
            matches_with_two += 1
            if any_lhb:
                matches_with_lhb += 1
    openers_bat = {
        "lhb_present_pct": (100 * matches_with_lhb / matches_with_two
                            if matches_with_two else 0),
        "n_matches_with_data": matches_with_two,
        "coverage_pct": (100 * coverage_known / coverage_total
                         if coverage_total else 0),
    }

    # ---- Openers: bowling -----------------------------------------------
    open_bowl = conn.execute(f"""
        SELECT match_id, bowl_position, bowler_id
        FROM bowling
        WHERE match_id IN ({mid_ph})
          AND team_bowling_club_id = ?
          AND bowl_position IN (1,2)
    """, (*match_ids, club_id)).fetchall()
    by_m = {}
    for mid, pos, pid in open_bowl:
        by_m.setdefault(mid, {})[pos] = pid
    matches_with_two = 0
    matches_with_left = 0
    matches_with_spin = 0
    ...
    for mid, pp in by_m.items():
        ...
        for pid in ids:
            ...
            arm = meta.get("bowling_arm")
            cat = bowling_category(bowling_tag(meta))
            if arm in ("right", "left") and cat in ("pace", "spin"):
                n_known += 1
                if arm == "left":
                    any_left = True
                if cat == "spin":
        ...
        if n_known == 2:
            ...
            if any_spin:
                matches_with_spin += 1
    openers_bowl = {
        "left_arm_present_pct": (100 * matches_with_left / matches_with_two
                                  if matches_with_two else 0),
        "spin_present_pct": (100 * matches_with_spin / matches_with_two
                              if matches_with_two else 0),
        "n_matches_with_data": matches_with_two,
        "coverage_pct": (100 * coverage_known / coverage_total
                         if coverage_total else 0),
    ...
    return {
        "match_n": len(match_ids),
        "bowl_mix": bowl_mix,
        "bowl_granular": bowl_granular,
        "lineup": lineup,
        "openers_bat": openers_bat,
        "openers_bowl": openers_bowl,
    }
...
def main():
    ...
    # Player-style metadata, keyed by player_id. Empty-but-OK if no
    # submissions have been approved yet.
    player_meta = load_player_metadata()

    # Team-level style profile (pace/spin mix, lineup hand mix, opener
    # patterns) — degrades gracefully when player_meta is sparse.
    style_profile = team_style_profile(
...
def render_md(d):
    ...
    for i, r in enumerate(d["top_bowlers"], 1):
        ...
        btag = bowling_tag(pmeta.get(r["bowler_id"]))
        if btag:
            nm = f"{nm} `{btag}`"
    ...
    if sp:
        ...
        bm = sp["bowl_mix"]
        md.append(
            f"- Pace: **{bm['pct_pace']:.0f}%** · "
            f"Spin: **{bm['pct_spin']:.0f}%** · "
            f"Unknown: {bm['pct_unknown']:.0f}% "
            f"_(coverage {bm['coverage_pct']:.0f}%, "
            f"{bm['total_balls']} balls across {sp['match_n']} matches)_"
        )
        if sp["bowl_granular"]:
            md.append("")
            md.append("| Code | Style | Share |")
            md.append("|---|---|--:|")
            for g in sp["bowl_granular"]:
                md.append(
                    f"| `{g['code']}` | {BOWL_TAG_LABEL.get(g['code'], g['code'])} "
                    f"| {g['pct']:.0f}% |"
                )
        md.append("")
...
/* Bowling style mix + lineup profile */
.style-block{margin:4px 0 6px}
.split-bar{height:24px;border-radius:7px;overflow:hidden;display:flex;
  background:#eef0f6;box-shadow:inset 0 0 0 1px rgba(0,0,0,.05);
  margin:6px 0 6px}
...
.split-bar .seg.pace{background:linear-gradient(180deg,#3884b0,#1d6488)}
.split-bar .seg.spin{background:linear-gradient(180deg,#d4a83a,#a47d20)}
.split-bar .seg.unk{background:linear-gradient(180deg,#aab1bd,#7f8694);
  color:#fff;opacity:.85}
.granular{margin:6px 0 0}
.gran-row{display:grid;
  grid-template-columns:46px 1fr 80px 32px;
  gap:8px;align-items:center;font-size:11px;margin:3px 0}
.gran-row .code{justify-self:start;margin:0}
.gran-row .lbl{color:var(--ink);font-weight:600;
  white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.gran-row .bar{height:9px;background:#eef0f6;border-radius:3px;
  overflow:hidden;position:relative}
.gran-row .bar .fill{display:block;height:100%}
.gran-row .bar .fill.pace{background:linear-gradient(180deg,#3884b0,#1d6488)}
.gran-row .bar .fill.spin{background:linear-gradient(180deg,#d4a83a,#a47d20)}
.gran-row .num{text-align:right;color:var(--muted);font-weight:700;
  font-size:10.5px;font-variant-numeric:tabular-nums}

.lp-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:6px 0}
.lp-tile{background:#fff;border:1px solid var(--line);border-radius:10px;
  padding:10px 12px;text-align:left;box-shadow:var(--shadow);
  display:flex;flex-direction:column;gap:2px}
.lp-tile .big{font-size:22px;font-weight:800;color:var(--ink);
  line-height:1.05;font-variant-numeric:tabular-nums}
.lp-tile .lbl{font-size:10.5px;font-weight:700;text-transform:uppercase;
...
.cov{display:inline-block;font-size:9.5px;font-weight:800;
  letter-spacing:.05em;text-transform:uppercase;
  padding:1px 6px;border-radius:5px}
.cov.ok{background:#e3f1e6;color:#15803d}
.cov.warn{background:#fbf1d8;color:#a3791b}
.cov.low{background:#f6dede;color:#8b1c1c}

.style-tag{display:inline-block;padding:1px 6px;border-radius:5px;
  font-size:9.5px;font-weight:800;letter-spacing:.04em;
  vertical-align:middle;margin-left:5px;background:#eef0f6;color:var(--muted);
  font-variant-numeric:tabular-nums}
.style-tag.bat-rhb{background:#dde6f6;color:#1d4ed8}
.style-tag.bat-lhb{background:#f5e2dd;color:#a3491b}
.style-tag.bowl-pace{background:#ddebf3;color:#1d6488}
.style-tag.bowl-spin{background:#f5ecda;color:#7a5300}
...
def render_html(d):
    ...
    for i, r in enumerate(d["top_batters"], 1):
        ...
        bt = batting_tag(d.get("player_meta", {}).get(bid))
        tag_html = (f" <span class='style-tag bat-{bt.lower()}'>{bt}</span>"
                    if bt else "")
        parts.append(
            f"<tr class='p-name{rank_cls}'>"
            f"<td colspan='10' class='l'>"
            f"<span class='rk'>{i}</span>"
            f"<span class='nm'>{nm_html}{tag_html}</span></td></tr>"
    ...
    for i, r in enumerate(d["top_bowlers"], 1):
        ...
        nm_html = _player_link(d.get("pc_subdomain"), r['bowler_id'],
                               "bowling", r['name'])
        btag = bowling_tag(d.get("player_meta", {}).get(r['bowler_id']))
        bcat = bowling_category(btag) if btag else None
        tag_html = (f" <span class='style-tag bowl-{bcat}'>{btag}</span>"
                    if btag else "")
        parts.append(
            f"<tr class='p-name{rank_cls}'>"
            f"<td colspan='10' class='l'>"
...
def _coverage_pill(pct):
    """Small inline pill summarising coverage % — colour-coded."""
    if pct is None:
        pct = 0
    if pct >= 70:
        cls = "ok"
        ...
        cls = "low"
    return f"<span class='cov {cls}'>{pct:.0f}% covered</span>"


# Compact human label for each bowling-tag code.
BOWL_TAG_LABEL = {
    "RF": "Right-arm fast",        "RMF": "Right-arm medium-fast",
    "RM": "Right-arm medium",
    "LF": "Left-arm fast",         "LMF": "Left-arm medium-fast",
    "LM": "Left-arm medium",
    "OS": "Right-arm off-spin",    "LS": "Right-arm leg-spin",
    "SLA": "Slow left-arm orthodox","LAU": "Left-arm wrist-spin",
}


def _render_bowl_mix(profile, club_name):
    """Pace vs spin split bar + granular type breakdown."""
    bm = profile.get("bowl_mix") or {}
    parts = ["<div class='style-block'>"]
    total = bm.get("total_balls", 0)
    if not total:
        parts.append("<p class='subtle'>No bowling data in scope.</p>")
        parts.append("</div>")
        return "".join(parts)

    pp = bm["pct_pace"]; ps = bm["pct_spin"]; pu = bm["pct_unknown"]
    parts.append("<div class='subtle'>"
                 f"How <b>{_esc(club_name)}</b>'s bowling overs split by "
                 f"style across {profile['match_n']} matches. "
                 f"{_coverage_pill(bm['coverage_pct'])}</div>")
    parts.append("<div class='split-bar' role='img' aria-label='pace vs spin'>")
    if pp > 0:
        parts.append(f"<div class='seg pace' style='width:{pp:.1f}%'>"
                     f"Pace {pp:.0f}%</div>")
    if ps > 0:
        parts.append(f"<div class='seg spin' style='width:{ps:.1f}%'>"
                     f"Spin {ps:.0f}%</div>")
    if pu > 0:
        parts.append(f"<div class='seg unk' style='width:{pu:.1f}%'>"
                     f"Unknown {pu:.0f}%</div>")
    parts.append("</div>")

    # Granular bars
    parts.append("<div class='granular'>")
    if not profile.get("bowl_granular"):
        parts.append("<p class='subtle' style='margin:4px 0 0'>"
                     "No tagged bowlers in this window — granular split "
                     "needs player-style metadata captured.</p>")
    else:
        for g in profile["bowl_granular"]:
            label = BOWL_TAG_LABEL.get(g["code"], g["code"])
            cat = g["category"]
            parts.append(
                "<div class='gran-row'>"
                f"<span class='code style-tag bowl-{cat}'>{g['code']}</span>"
                f"<span class='lbl'>{_esc(label)}</span>"
                f"<span class='bar'><span class='fill {cat}' "
                f"style='width:{g['pct']:.1f}%'></span></span>"
                f"<span class='num'>{g['pct']:.0f}%</span>"
                "</div>"
            )
