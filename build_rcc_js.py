...
Read by `build_rcc_dashboard.build()`, written verbatim to
`app/static/rcc.js`. Kept in its own module so the JS body is easy
to edit without scrolling past the Python data plumbing.

The page is `app/rcc/player.html?id=<player_id>`. The JS:

  1. Pulls the per-player JSON from `../data/rcc/<id>.json`
  2. Renders a slicer rail: season / home-away / result /
     competition / opp-club / position / bat-1st-2nd / toss / phase /
     player-innings-bucket / bowler-type / bowler-arm / spell / batter-
     hand. All multi-select within a slicer; all slicers AND together.
  3. Recomputes the dashboard cards on every toggle.
  4. Mirrors filter state into the URL hash (so it's shareable).
  5. Decorates every BBB-derived card with a volume chip
     (🎯 N balls) so the user sees how thin the slice is.

Every aggregator mirrors the maths in `_bbb_buckets.py`.
"""
